...
pbxProject.prototype.addHeaderFile = function (path, opt, group) {
    ...
    this.addToPbxBuildFileSection(file);        // PBXBuildFile
    this.addToPbxHeadersBuildPhase(file);       // PBXHeadersBuildPhase
...
pbxProject.prototype.removeHeaderFile = function (path, opt, group) {
    ...
    file.target = opt ? opt.target : undefined;
    this.removeFromPbxBuildFileSection(file);        // PBXBuildFile
    this.removeFromPbxHeadersBuildPhase(file);       // PBXHeadersBuildPhase
...
pbxProject.prototype.addToPbxHeadersBuildPhase = function(file) {
    var headers = this.pbxHeadersBuildPhaseObj(file.target);
    headers.files.push(pbxBuildPhaseObj(file));
}

pbxProject.prototype.removeFromPbxHeadersBuildPhase = function(file) {
    var headers = this.pbxHeadersBuildPhaseObj(file.target), i;
    for (i in headers.files) {
        if (headers.files[i].comment == longComment(file)) {
            headers.files.splice(i, 1);
            break;
        }
    }
}
...
pbxProject.prototype.addTargetDependency = function(target, dependencyTargets) {
    ...
    for (var index = 0; index < dependencyTargets.length; index++) {
        ...
        for (id in files) {
            file = files[id];
            if (file.name === dependencyTargets[index]) {
                break;
            }
        }

        var dependencyTargetUuid = this.generateUuid(),
            ...
            targetDependency = {
                ...
                name: path.basename(file.path).replace('.xcodeproj"', ''),
...
pbxProject.prototype.pbxHeadersBuildPhaseObj = function (target) {
...
pbxProject.prototype.getFileUuid = function(name) {
    var files = nonComments(this.pbxFileReferenceSection()), file, id;
    for (id in files) {
        file = files[id];
        if (`"${name}"` === file.name) {
...
pbxProject.prototype.addCopyfileFrameworkCustom = function(name) {
    var file = new pbxFile(name);

    file.uuid = this.generateUuid();
    file.fileRef = this.getFileUuid(name);
    file.group = 'CopyFiles';
    this.addToPbxBuildFileSection(file);        // PBXBuildFile

    var sources = this.buildPhaseObject('PBXCopyFilesBuildPhase', 'CopyFiles', this.findTargetKey('ElectrodeContainer'));
    sources.files.push(pbxBuildPhaseObj(file));
}
